...
def print_progress(progress):
    ...
    if isinstance(progress, CrawlProgress):
        ...
        print(
            f"\r[{pct:5.1f}%] {progress.stage}: {progress.message}", end="", flush=True
        )
...
def cmd_sync(args):
    ...
    try:
        ...
        progress = run_sync(service, db_path=db_path, progress_callback=print_progress)
    ...
    if args.json:
        ...
        if result.details.get("dangling_edges", {}).get("missing_parent_count", 0) > 0:
            ...
            if len(result.details["dangling_edges"]["missing_parents"]) > 10:
                print(
                    f"  ... and {len(result.details['dangling_edges']['missing_parents']) - 10} more"
                )
        ...
        if result.details.get("shortcuts", {}).get("unresolved_count", 0) > 0:
            ...
            if len(result.details["shortcuts"]["unresolved"]) > 10:
                print(
                    f"  ... and {len(result.details['shortcuts']['unresolved']) - 10} more"
                )
...
def cmd_dedupe_report(args):
    ...
    with get_connection(db_path) as conn:
        ...
        print(
            f"  Potential savings: {savings['total_wasted_bytes'] / (1024**3):.2f} GB"
        )
        print()

        if args.json:
            # Output as JSON
            output = {"summary": savings, "groups": []}
            for group in groups:
...
Examples:
  ...
  python -m backend.cli dedupe_report --min-size 1000000
    ...
    crawl_parser.add_argument(
        "--skip-health", action="store_true", help="Skip health checks after crawl"
    )
    ...
    dedupe_parser = subparsers.add_parser(
        "dedupe_report", help="Generate duplicate report"
    )
    ...
    dedupe_parser.add_argument("--json", action="store_true", help="Output as JSON")
