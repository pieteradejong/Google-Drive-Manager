...
def get_duplicate_files_detail(
    conn,
    file_ids: List[str]
) -> List[Dict[str, Any]]:
    """
...
def calculate_folder_sizes(files: List[Dict[str, Any]], children_map: Dict[str, List[str]]) -> None:
    """
...
def build_scan_response_data(db_path: Optional[Path] = None) -> Dict[str, Any]:
    ...
    with get_connection(path) as conn:
        ...
        files_only = [f for f in files if f["mimeType"] != "application/vnd.google-apps.folder"]
        
        total_size = sum(
            f.get("calculatedSize") or f.get("size") or 0
            for f in files
        )
