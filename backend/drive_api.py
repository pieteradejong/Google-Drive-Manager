...
def build_tree_structure(all_files: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    total_duration_ms = (time.perf_counter() - start_time) * 1000
    folder_count = len(
        [f for f in all_files if f["mimeType"] == "application/vnd.google-apps.folder"]
    )
...
def get_top_level_folders(service) -> tuple[List[Dict[str, Any]], Optional[int]]:
    ...
    with log_timing("get_top_level_folders.estimate"):
        first_page = (
            service.files()
            .list(q="trashed=false", pageSize=1000, fields="nextPageToken, files(id)")
            .execute()
        )
...
def check_recently_modified(
    service, since_timestamp: datetime, limit: int = 1
) -> List[Dict[str, Any]]:
    """
    ...
    try:
        ...
        perf_logger.info(
            ...
            cache_age_days=round(
                (datetime.now(timezone.utc) - since_timestamp).total_seconds() / 86400,
                1,
            ),
        )
        return files
...
def get_start_page_token(service) -> str:
    ...
    try:
        response = (
            service.changes().getStartPageToken(supportsAllDrives=False).execute()
        )
        ...
        duration_ms = (time.perf_counter() - start_time) * 1000
        perf_logger.error(
            "get_start_page_token", duration_ms=duration_ms, message=f"Error: {str(e)}"
        )
        raise
