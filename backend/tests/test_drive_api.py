...
class TestGetDriveOverview:
    ...
    def test_get_drive_overview_success(self, mock_about_response):
        ...
        service = MagicMock()
        service.about.return_value.get.return_value.execute.return_value = mock_about_response
    ...
    def test_get_drive_overview_calls_api_correctly(self):
        ...
        service.about.return_value.get.assert_called_once_with(fields="storageQuota,user")
...
class TestGetTopLevelFolders:
    ...
    def test_get_top_level_folders_with_pagination(self):
        ...
        page1_mock.execute.return_value = {
            'files': [{'id': 'folder1', 'name': 'F1', 'mimeType': 'application/vnd.google-apps.folder'}],
            'nextPageToken': 'page2'
        }
        ...
        page2_mock.execute.return_value = {
            'files': [{'id': 'folder2', 'name': 'F2', 'mimeType': 'application/vnd.google-apps.folder'}],
            'nextPageToken': None
        }
        
        service.files.return_value.list.side_effect = [first_page_mock, page1_mock, page2_mock]
    ...
    def test_get_top_level_folders_empty(self):
        ...
        empty_mock.execute.return_value = {
            'files': [],
            'nextPageToken': None
        }
...
class TestGetStartPageToken:
    ...
    def test_get_start_page_token_error(self):
        """Test error handling."""
        service = MagicMock()
        service.changes.return_value.getStartPageToken.return_value.execute.side_effect = Exception("API Error")
...
class TestListChanges:
    ...
    def test_list_changes_success(self, mock_changes_response):
        ...
        service = MagicMock()
        service.changes.return_value.list.return_value.execute.return_value = mock_changes_response
