...
class TestRunSync:
    ...
    def test_run_sync_processes_removed_files(self, populated_db):
        ...
        with get_connection(populated_db) as conn:
            upsert_file(conn, {'id': 'to_remove', 'name': 'Remove.txt', 'mimeType': 'text/plain'})
            conn.commit()
        
        changes = [
            {'fileId': 'to_remove', 'removed': True}
        ]
    ...
    def test_run_sync_updates_token(self, populated_db, mock_changes_response):
        ...
        with patch('backend.sync_changes.list_changes') as mock_list:
            mock_list.return_value = (mock_changes_response['changes'], 'brand_new_token')
...
class TestSmartSync:
    ...
    def test_smart_sync_with_progress_callback(self, populated_db, mock_changes_response):
