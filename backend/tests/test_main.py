...
class TestScanEndpoint:
    ...
    @patch("backend.main.get_service")
    def test_scan_endpoint_authentication_error(self, mock_get_service, client):
...
class TestAnalyticsStatusEndpoint:
    ...
    def test_analytics_status_missing_full_scan(
        self, mock_analytics_meta, mock_full_meta, client
    ):
    ...
    def test_analytics_status_ready(
        self, mock_is_valid, mock_analytics_meta, mock_full_meta, client
    ):
        """Test status when analytics cache is ready."""
        mock_full_meta.return_value = CacheMetadata(
            timestamp=datetime.now(timezone.utc).isoformat(), cache_version=1
        )
    ...
    @patch("backend.main._current_full_scan_cache_metadata")
    def test_analytics_status_running(self, mock_full_meta, client):
...
class TestAnalyticsStartEndpoint:
    ...
    def test_analytics_start_no_full_scan(
        self, mock_full_meta, mock_start_compute, client
    ):
...
class TestAnalyticsViewEndpoint:
    ...
    @patch("backend.main._current_full_scan_cache_metadata")
    def test_analytics_view_no_full_scan(self, mock_full_meta, client):
    ...
    def test_analytics_view_duplicates(
        ...
        mock_full_meta.return_value = CacheMetadata(
            timestamp=timestamp, cache_version=1
        )
        mock_analytics_meta.return_value = AnalyticsCacheMetadata(
        ...
        mock_is_valid.return_value = True
        mock_build_index.return_value = ({"files": []}, {})
    ...
    def test_analytics_view_unknown(
        ...
        mock_load_cache.return_value = {"data": {"derived_version": 1}}
        mock_is_valid.return_value = True
...
class TestIndexStatusEndpoint:
    ...
    def test_index_status_not_initialized(self, mock_db_exists, client):
    ...
    def test_index_status_empty(self, mock_get_info, mock_db_exists, client):
...
class TestIndexCrawlEndpoint:
    ...
    def test_index_crawl_start(self, mock_thread, mock_get_service, client):
...
class TestIndexSyncEndpoint:
    ...
    @patch("backend.sync_changes.can_sync")
    def test_index_sync_not_available(self, mock_can_sync, client):
    ...
    def test_index_sync_start(
        self, mock_thread, mock_get_service, mock_can_sync, client
    ):
...
class TestIndexDataEndpoint:
    ...
    @patch("backend.index_db.database_exists")
    def test_index_data_no_database(self, mock_db_exists, client):
    ...
    def test_index_data_success(
        self, mock_build_response, mock_db_exists, client, sample_files
    ):
...
class TestIndexDuplicatesEndpoint:
    ...
    @patch("backend.index_db.database_exists")
    def test_index_duplicates_no_database(self, mock_db_exists, client):
    ...
    def test_index_duplicates_success(
        ...
        mock_get_groups.return_value = [
            {"name": "Report.pdf", "size": 5000, "file_ids": ["f1", "f2"], "count": 2}
        ]
...
class TestIndexClearEndpoint:
    ...
    def test_index_clear_success(self, mock_clear, client):
    ...
    @patch("backend.index_db.clear_database")
    def test_index_clear_error(self, mock_clear, client):
...
class TestCachedFullScanEndpoint:
    ...
    @patch("backend.main.load_cache")
    def test_cached_full_scan_no_cache(self, mock_load, client):
    ...
    def test_cached_full_scan_expired(
        self, mock_validate, mock_service, mock_load, client
    ):
