...
class TestCacheValidation:
    ...
    def test_validate_cache_with_drive_no_changes(self, mock_check_recently):
        ...
        result = validate_cache_with_drive(
            mock_service, metadata, max_age_seconds=604800
        )
    ...
    def test_validate_cache_with_drive_has_changes(self, mock_check_recently):
        ...
        metadata = CacheMetadata(timestamp=past.isoformat())
        mock_service = MagicMock()
        mock_check_recently.return_value = [
            {"id": "123", "name": "modified.txt"}
        ]  # Files modified
...
class TestCacheManagement:
    ...
    def test_clear_cache_specific(self, tmp_path):
        ...
        with patch("backend.cache.get_cache_path", return_value=cache_file), patch(
            "backend.cache.get_cache_metadata_path", return_value=meta_file
        ):
    ...
    def test_get_cache_metadata_success(self, mock_load_cache_metadata):
        ...
        mock_load_cache_metadata.return_value = CacheMetadata(
            timestamp="2024-01-15T10:30:00Z", file_count=100, cache_version=1
        )
