...
class TestInitDatabase:
    ...
    def test_init_db_creates_tables(self, temp_db_path):
        ...
        with get_connection(temp_db_path) as conn:
            ...
            cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='file_errors'")
            assert cursor.fetchone() is not None
...
class TestParentEdges:
    ...
    def test_replace_parents_insert(self, initialized_db):
        ...
        with get_connection(initialized_db) as conn:
            # Create file first
            upsert_file(conn, {'id': 'child1', 'name': 'Child', 'mimeType': 'text/plain'})
    ...
    def test_replace_parents_replaces_existing(self, initialized_db):
        ...
        with get_connection(initialized_db) as conn:
            upsert_file(conn, {'id': 'child2', 'name': 'Child', 'mimeType': 'text/plain'})
...
class TestMarkFileRemoved:
    ...
    def test_mark_file_removed_sets_flag(self, initialized_db):
        ...
        with get_connection(initialized_db) as conn:
            upsert_file(conn, {'id': 'remove1', 'name': 'ToRemove', 'mimeType': 'text/plain'})
            conn.commit()
    ...
    def test_mark_file_removed_deletes_parent_edges(self, initialized_db):
        ...
        with get_connection(initialized_db) as conn:
            upsert_file(conn, {'id': 'remove2', 'name': 'ToRemove', 'mimeType': 'text/plain'})
    ...
    def test_removed_files_excluded_from_queries(self, initialized_db):
        ...
        with get_connection(initialized_db) as conn:
            upsert_file(conn, {'id': 'visible', 'name': 'Visible', 'mimeType': 'text/plain'})
            upsert_file(conn, {'id': 'removed', 'name': 'Removed', 'mimeType': 'text/plain'})
...
class TestGetAllFiles:
    ...
    def test_get_all_files_excludes_trashed_by_default(self, initialized_db):
        ...
        with get_connection(initialized_db) as conn:
            upsert_file(conn, {'id': 'normal', 'name': 'Normal', 'mimeType': 'text/plain', 'trashed': False})
            upsert_file(conn, {'id': 'trashed', 'name': 'Trashed', 'mimeType': 'text/plain', 'trashed': True})
            conn.commit()
