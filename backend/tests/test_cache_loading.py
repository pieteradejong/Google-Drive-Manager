...
class TestCacheLoading:
    ...
    def test_cache_validity_check(self):
        ...
        assert is_cache_valid_time_based(expired_metadata, max_age_seconds=3600) is False
    ...
    def test_cache_with_drive_validation(self):
        ...
        with patch('backend.drive_api.check_recently_modified', return_value=[]):
            ...
            result = validate_cache_with_drive(mock_service, old_metadata, max_age_seconds=604800)
            assert result is True
        
        # Cache that's past TTL and Drive has changed
        with patch('backend.drive_api.check_recently_modified', return_value=[{"id": "file1", "name": "New File"}]):
