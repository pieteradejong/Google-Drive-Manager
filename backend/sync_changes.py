...
def run_sync(
    ...
    try:
        ...
        if not start_token:
            raise RuntimeError(
                "No start_page_token found. Run a full crawl first."
            )
        
        sync_logger.info(
            "run_sync.start",
            token_prefix=start_token[:10] if start_token else None
        )
        ...
        with get_connection(path) as conn:
            for i, change in enumerate(all_changes):
                try:
                    ...
                    if (i + 1) % 100 == 0:
                        conn.commit()
                        progress.message = f"Processed {i + 1}/{progress.total_changes} changes..."
                        update_progress()
